const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'New OpenAI 賬號',
    langGeniusAccountTip: '您的 New OpenAI 賬號和相關的使用者資料。',
    editName: '編輯名字',
  ...
  members: {
    ...
    invitationSentTip: '邀請已傳送，對方登入 New OpenAI 後即可訪問你的團隊資料。',
    invitationLink: '邀請連結',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '體驗',
      exhausted: '超出限額',
      desc: '託管 OpenAI 由 New OpenAI 提供的託管 OpenAI 服務，你可以使用 GPT-3.5 等模型，在體驗額度消耗完畢前你需要設定其它模型供應商。',
      callTimes: '呼叫次數',
  ...
  apiBasedExtension: {
    title: 'API 擴充套件提供了一個集中式的 API 管理，在此統一新增 API 配置後，方便在 New OpenAI 上的各類應用中直接使用。',
  ...
  about: {
    changeLog: '更新日誌',
    updateNow: '現在更新',
    nowAvailable: 'New OpenAI {{version}} 現已可用。',
